...
export function BrowserUpgradeNotice({ currentVersion, onDismiss, className }: BrowserUpgradeNoticeProps) {
  const handleUpgradeClick = () => {
    chrome.tabs.create({ url: 'https://github.com/browsergenie-ai/BrowserGenie/releases/latest' })
  }
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-foreground">
          BrowserGenie Update Required
        </p>
        <p className="mt-0.5 text-xs text-muted-foreground">
          You're using BrowserGenie {currentVersion || 'an older version'}. Please upgrade to use Teach mode.
        </p>
      </div>
