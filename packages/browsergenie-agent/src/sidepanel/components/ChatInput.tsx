...
import { BrowserGenieProvidersConfig, BrowserGenieProvider } from '@/lib/llm/settings/BrowserGenieTypes'
...
export function ChatInput({ isConnected, isProcessing }: ChatInputProps) {
  ...
  useEffect(() => {
    const computeOk = (cfg: BrowserGenieProvidersConfig) => {
    ...
    const handleWorkflow = (payload: any) => {
      ...
      if (payload && payload.data && payload.data.providersConfig) {
        computeOk(payload.data.providersConfig as BrowserGenieProvidersConfig)
      }
    }
  ...
  const isProviderUsable = (provider: BrowserGenieProvider | null): boolean => {
