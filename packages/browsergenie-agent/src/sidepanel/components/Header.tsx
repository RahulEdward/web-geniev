...
import { z } from 'zod'
import { BrowserGenieProvidersConfig, BrowserGenieProvidersConfigSchema } from '@/lib/llm/settings/types'
import { MCP_SERVERS, type MCPServerConfig } from '@/config/mcpServers'

const GITHUB_REPO_URL: string = 'https://github.com/browsergenie-ai/BrowserGenie'
...
export const Header = memo(function Header({ onReset, showReset, isProcessing, isTeachMode = false }: HeaderProps) {
  ...
  const [providersConfig, setProvidersConfig] = useState<BrowserGenieProvidersConfig | null>(null)
    ...
    try {
      ...
      try {
        await chrome.tabs.create({
          url: chrome.runtime.getURL('BrowserGenie-settings.html')
        })
      } catch (fallbackError) {
    ...
    const handler = (payload: any) => {
      if (payload && payload.status === 'success' && payload.data) {
        ...
        if (payload.data.providersConfig) {
          try {
            const cfg = BrowserGenieProvidersConfigSchema.parse(payload.data.providersConfig)
