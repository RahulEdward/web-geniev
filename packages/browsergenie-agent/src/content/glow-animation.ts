...
(() => {
  const GLOW_OVERLAY_ID = 'BrowserGenie-glow-overlay'
  const GLOW_STYLES_ID = 'BrowserGenie-glow-styles'
  const GLOW_INITIALIZED_KEY = 'BrowserGenie-glow-initialized'
  const GLOW_ENABLED_KEY = 'BrowserGenie-glow-enabled'  // Stored in chrome.storage.local
  ...
  if ((window as any)[GLOW_INITIALIZED_KEY]) {
    console.log('[BrowserGenie] Glow animation already initialized')
    return
  }
  ...
  function injectStyles(): void {
    ...
    style.textContent = `
      @keyframes BrowserGenie-glow-pulse {
        0% {
          box-shadow:
      ...
      @keyframes BrowserGenie-glow-fade-in {
        from { opacity: 0; }
      ...
      #${GLOW_OVERLAY_ID} {
        ...
        will-change: opacity;
        animation:
          BrowserGenie-glow-pulse 3s ease-in-out infinite,
          BrowserGenie-glow-fade-in 420ms cubic-bezier(0.22, 1, 0.36, 1) forwards !important;
      }
    `
  ...
  function startGlow(): void {
    ...
    console.log('[BrowserGenie] Glow animation started')
  }
  ...
  function stopGlow(): void {
    ...
    if (overlay) {
      overlay.remove()
      console.log('[BrowserGenie] Glow animation stopped')
    }
  }
