...
import { BrowserGenieProvidersConfigSchema, BrowserGenie_PREFERENCE_KEYS } from '@/lib/llm/settings/BrowserGenieTypes'
...
export class ProvidersHandler {
  ...
  async handleSaveProviders(
    ...
    try {
      ...
      const config = BrowserGenieProvidersConfigSchema.parse(payload)
      ...
      const BrowserGenie = (chrome as any)?.BrowserGenie
      if (BrowserGenie?.setPref) {
        BrowserGenieSuccess = await new Promise<boolean>((resolve) => {
          BrowserGenie.setPref(key, configStr, undefined, (success: boolean) => {
            const error = chrome.runtime?.lastError
            if (error) {
              Logging.log('ProvidersHandler', `BrowserGenie setPref error: ${error.message}`, 'warning')
              resolve(false)
            } else if (success) {
              resolve(true)
            } else {
              Logging.log('ProvidersHandler', 'BrowserGenie setPref returned false', 'warning')
              resolve(false)
            }
          })
        })
