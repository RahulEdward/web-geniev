...
export class SettingsHandler {
  async handleGetPref(message: PortMessage, port: chrome.runtime.Port): Promise<void> {
    ...
    const BrowserGeniePrefs = (chrome as any)?.BrowserGenie
    if (BrowserGeniePrefs?.getPrefs) {
      try {
        BrowserGeniePrefs.getPrefs([name], (prefs: Record<string, unknown>) => {
          const error = chrome.runtime?.lastError
          if (error) {
            Logging.log('SettingsHandler', `BrowserGenie getPrefs error for ${name}: ${error.message}`, 'error')
            port.postMessage({
        ...
        Logging.log('SettingsHandler', `Error getting pref via BrowserGenie ${name}: ${error}`, 'error')
        port.postMessage({
          type: MessageType.ERROR,
  ...
  async handleSetPref(message: PortMessage, port: chrome.runtime.Port): Promise<void> {
    ...
    const BrowserGeniePrefs = (chrome as any)?.BrowserGenie
    if (BrowserGeniePrefs?.setPrefs) {
      try {
        BrowserGeniePrefs.setPrefs({ [name]: value }, (success?: boolean) => {
          const error = chrome.runtime?.lastError
          if (error) {
            Logging.log('SettingsHandler', `BrowserGenie setPrefs error for ${name}: ${error.message}`, 'error')
            port.postMessage({
          ...
          if (!ok) {
            Logging.log('SettingsHandler', `BrowserGenie setPrefs reported failure for ${name}`, 'error')
          }
        ...
        Logging.log('SettingsHandler', `Error setting pref via BrowserGenie ${name}: ${error}`, 'error')
        port.postMessage({
          type: MessageType.ERROR,
    ...
    try {
      ...
      try {
        ...
        switch (provider.type) {
          ...
          case 'BrowserGenie':
            llm = new ChatOpenAI({
              openAIApiKey: 'BrowserGenie-key',
              modelName: 'default-llm',
              temperature: 0.7,
              maxTokens: 100,
              streaming: false,
              configuration: {
                baseURL: 'https://llm.browsergenie.com/default/'
              }
            })
            break
