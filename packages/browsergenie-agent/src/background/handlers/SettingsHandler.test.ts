...
// Mock Chrome BrowserGenie prefs API
const mockBrowserGeniePrefs = {
  getPrefs: vi.fn(),
  setPrefs: vi.fn()
}
...
describe('SettingsHandler-unit-test', () => {
  ...
  beforeEach(() => {
    ...
    ;(globalThis as any).chrome = {
      BrowserGenie: mockBrowserGeniePrefs
    }
  })
  ...
  it('tests that handleGetPref calls chrome.BrowserGenie.getPrefs correctly', async () => {
    mockBrowserGeniePrefs.getPrefs.mockImplementation((keys: string[], callback) => {
    ...
    // Verify BrowserGenie API was called
    expect(mockBrowserGeniePrefs.getPrefs).toHaveBeenCalledWith(['test-pref'], expect.any(Function))
  ...
  it('tests that handleSetPref calls chrome.BrowserGenie.setPrefs correctly', async () => {
    mockBrowserGeniePrefs.setPrefs.mockImplementation((prefs: Record<string, unknown>, callback) => {
      callback?.(true)
    })
    ...
    // Verify BrowserGenie API was called
    expect(mockBrowserGeniePrefs.setPrefs).toHaveBeenCalledWith({ 'test-pref': 'test-value' }, expect.any(Function))
  ...
  it('tests that error handling works when chrome APIs throw', async () => {
    mockBrowserGeniePrefs.getPrefs.mockImplementation(() => {
