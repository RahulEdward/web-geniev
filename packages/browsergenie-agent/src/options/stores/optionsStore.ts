...
} from '@/lib/llm/settings/BrowserGenieTypes'


interface OptionsStore {
  providers: BrowserGenieProvider[]
  ...
  setProviders: (providers: BrowserGenieProvider[]) => Promise<void>
  setDefaultProvider: (providerId: string) => Promise<void>
  addProvider: (provider: BrowserGenieProvider) => Promise<void>
  updateProvider: (provider: BrowserGenieProvider) => Promise<void>
...
const readProvidersFromPrefs = async (): Promise<BrowserGenieProvidersConfig | null> => {
  ...
  if ((chrome as any)?.BrowserGenie?.getPref) {
    try {
      const pref = await new Promise<any>((resolve, reject) => {
        (chrome as any).BrowserGenie.getPref(key, (pref: any) => {
  ...
  // Merge both configs
  const mergedConfig = mergeProviderConfigs(BrowserGenieConfig, storageLocalConfig)
...
const writeProvidersToPrefs = async (config: BrowserGenieProvidersConfig): Promise<boolean> => {
  const key = BrowserGenie_PREFERENCE_KEYS.PROVIDERS
  ...
  if ((chrome as any)?.BrowserGenie?.setPref) {
    BrowserGenieSuccess = await new Promise<boolean>((resolve) => {
      (chrome as any).BrowserGenie.setPref(key, configJson, undefined, (success?: boolean) => {
        const error = chrome.runtime?.lastError
        if (error) {
          console.warn('[optionsStore] BrowserGenie setPref error:', error.message)
          resolve(false)
        } else if (success !== false) {
          resolve(true)
        } else {
          console.warn('[optionsStore] BrowserGenie setPref returned false')
          resolve(false)
        }
      })
    })
...
export const useOptionsStore = create<OptionsStore>((set, get) => ({
  providers: [],
  defaultProviderId: 'BrowserGenie',
  isLoading: false,
  error: null,
  ...
  setProviders: async (providers) => {
    ...
    if (!providers.find(p => p.id === defaultProviderId)) {
      finalDefaultId = providers[0]?.id || 'BrowserGenie'
    }
  ...
  deleteProvider: async (providerId) => {
    ...
    if (providerId === defaultProviderId) {
      newDefaultId = updatedProviders[0]?.id || 'BrowserGenie'
    }
