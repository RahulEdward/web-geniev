...
import { BrowserGenieProvidersConfig } from '@/lib/llm/settings/BrowserGenieTypes'

const HEARTBEAT_INTERVAL_MS = 20000  // Send heartbeat every 20 seconds to keep background alive

const DEFAULT_BrowserGenie_PROVIDER: LLMProvider = {
  id: 'BrowserGenie',
  name: 'BrowserGenie',
  type: 'BrowserGenie',
  isBuiltIn: true,
...
export function useBrowserGeniePrefs() {
  const [providers, setProviders] = useState<LLMProvider[]>([DEFAULT_BrowserGenie_PROVIDER])
  const [defaultProvider, setDefaultProviderState] = useState<string>('BrowserGenie')
  ...
  const ensurePortConnected = useCallback(() => {
    if (!port || !isPortConnected) {
      console.warn('[useBrowserGeniePrefs] Port not connected')
      return null
    }
    return port
  ...
  useEffect(() => {
    ...
    const setupPort = () => {
      try {
        ...
        messageListener = (msg: PortMessage) => {
          ...
          if (msg.type === MessageType.WORKFLOW_STATUS) {
            ...
            if (payload?.status === 'error') {
              console.error('[useBrowserGeniePrefs] Error from background:', payload.error)
            }
            if (payload?.data?.providersConfig) {
              const config = payload.data.providersConfig as BrowserGenieProvidersConfig
              ...
              setDefaultProviderState(config.defaultProviderId || 'BrowserGenie')
              setIsLoading(false)
            }
          }
        }
        ...
        disconnectListener = () => {
          console.warn('[useBrowserGeniePrefs] Port disconnected')
        ...
        heartbeatInterval = setInterval(() => {
          if (currentPort) {
            try {
              ...
              console.warn('[useBrowserGeniePrefs] Heartbeat failed, port likely disconnected')
              if (heartbeatInterval) {
                clearInterval(heartbeatInterval)
                heartbeatInterval = null
              }
        ...
        const initialTimeout = setTimeout(() => {
          if (currentPort) {
            try {
              ...
              console.error('[useBrowserGeniePrefs] Failed to send initial message:', error)
            }
          }
        }, 100)
        ...
        console.error('[useBrowserGeniePrefs] Failed to setup port:', error)
        setIsPortConnected(false)
        return () => {}
      }
    }
  ...
  const saveProvidersConfig = useCallback(async (updatedProviders: LLMProvider[], newDefaultId?: string) => {
    ...
    if (!connectedPort) {
      console.error('[useBrowserGeniePrefs] Port not connected, cannot save providers')
      return false
    }

    const config: BrowserGenieProvidersConfig = {
    ...
    try {
      ...
      console.error('[useBrowserGeniePrefs] Failed to send save message:', error)
      setIsPortConnected(false)
      return false
    }
  ...
  const deleteProvider = useCallback(async (providerId: string) => {
    ...
    if (providerId === defaultProvider) {
      const BrowserGenieProvider = remainingProviders.find(p => p.id === 'BrowserGenie')
      nextDefaultId = BrowserGenieProvider?.id || remainingProviders[0]?.id || 'BrowserGenie'
