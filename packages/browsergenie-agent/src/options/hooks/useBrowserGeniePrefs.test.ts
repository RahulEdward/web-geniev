...
import { useBrowserGeniePrefs } from './useBrowserGeniePrefs'
...
describe('useBrowserGeniePrefs-unit-test', () => {
  it('tests that hook can be initialized with default state', () => {
    const { result } = renderHook(() => useBrowserGeniePrefs())

    expect(result.current.providers).toHaveLength(1)
    expect(result.current.providers[0].name).toBe('BrowserGenie')
    expect(result.current.providers[0].type).toBe('BrowserGenie')
    expect(result.current.defaultProvider).toBe('BrowserGenie')
    ...
    expect(mockPort.postMessage).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        payload: expect.objectContaining({
          name: 'BrowserGenie.llm_providers'
        })
      })
    )
  })
...
describe('useBrowserGeniePrefs-integration-test', () => {
  it.skipIf(!process.env.LITELLM_API_KEY || process.env.LITELLM_API_KEY === 'nokey')(
    ...
    async () => {
      ...
      console.log('✅ useBrowserGeniePrefs integration test passed')
    },
    30000
  )
})