...
import { useBrowserGeniePrefs } from './hooks/useBrowserGeniePrefs'
...
export function OptionsNew() {
  const { providers, defaultProvider, setDefaultProvider, addProvider, updateProvider, deleteProvider } = useBrowserGeniePrefs()
  const { theme } = useSettingsStore()

  // Get initial section from URL hash or default to 'browsergenie-ai'
  const getInitialSection = () => {
    const hash = window.location.hash.slice(1) // Remove '#'
    const validSections = ['browsergenie-ai', 'mcp', 'search-providers', 'about']
    return validSections.includes(hash) ? hash : 'browsergenie-ai'
  }
  ...
  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'BrowserGenie-settings' && e.newValue) {
        try {
