...
const DEFAULT_PROVIDERS: Provider[] = [
  {
    id: 'BrowserGenie-agent',
    name: 'BrowserGenie Agent',
    category: 'llm',
    actionType: 'sidepanel',
    available: true,
    iconUrl: '/assets/new_tab_search/BrowserGenie.svg'
  },
  {
    id: 'chatgpt',
    name: 'ChatGPT',
...
export const useProviderStore = create<ProviderState & ProviderActions>()(
  persist(
    (set, get) => ({
      ...
      customProviders: [],
      selectedProviderId: 'BrowserGenie-agent',
      isDropdownOpen: false,
      enabledProviderIds: ['BrowserGenie-agent', 'chatgpt', 'claude', 'google'],
      ...
      removeCustomProvider: id => {
        set(state => {
          ...
          const nextSelected = isSelected
            ? normalized.enabledProviderIds[0] || 'BrowserGenie-agent'
      ...
      importLegacyProviderSettings: () => {
        ...
        try {
          ...
          const mapLegacyId = (id: string) => (id === 'BrowserGenie' ? 'BrowserGenie-agent' : id)
      ...
      executeProviderAction: async (provider, query) => {
        ...
        if (provider.actionType === 'url' && provider.urlPattern) {
          ...
          const BrowserGenie = getBrowserGenieAdapter()
          ...
          try {
            ...
            if (needsDomWork && tabId != null) {
              ...
              while (Date.now() - start < CHAT_PROVIDER_READY_TIMEOUT_MS) {
                try {
                  const status = await BrowserGenie.getPageLoadStatus(tabId)
              ...
              if (!hasPlaceholder) {
                const injectQuery = async () => {
                  ...
                  try {
                    ...
                    const result = await BrowserGenie.executeJavaScript(tabId, script);
              ...
              if (provider.focusBeforeSubmit) {
                try {
                  await BrowserGenie.executeJavaScript(tabId, `
                    (function() {
            ...
            if (provider.autoSubmit && tabId != null) {
              if (hasPlaceholder || queryInjected) {
                ...
                await BrowserGenie.sendKeys(tabId, submitKey as chrome.BrowserGenie.Key)
              }
            }
          } catch (error) {
    ...
    }),
    {
      ...
      migrate: (persistedState, version) => {
        ...
        return {
          providers: DEFAULT_PROVIDERS,
          customProviders,
          selectedProviderId: legacyState.selectedProviderId || 'BrowserGenie-agent',
          isDropdownOpen: false,
          enabledProviderIds: normalized.enabledProviderIds,
          disabledProviderIds: normalized.disabledProviderIds,
          hasLegacySynced: false
