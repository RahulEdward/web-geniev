...
import { BrowserGenieProvider } from '@/lib/llm/settings/BrowserGenieTypes'
...
const BrowserGenie_DEFAULT_PROXY_URL = "https://llm.browsergenie.com/default/"
const BrowserGenie_FAST_LLM_PROXY_URL = "https://llm.browsergenie.com/fast/"
const BrowserGenie_SMART_LLM_PROXY_URL = "https://llm.browsergenie.com/smart/"
const DEFAULT_BrowserGenie_MODEL_FAMILY_URL = "https://llm.browsergenie.com/api/model_family"
const DEFAULT_BrowserGenie_MODEL = "openai"  // Fallback model family
...
// BrowserGenie API response structure
interface BrowserGenieModelConfig {
...
export class LangChainProvider {
  ...
  private currentProvider: BrowserGenieProvider | null = null
  ...
  // Model config cache for BrowserGenie - now stores full API response
  private modelConfigCache: { config: BrowserGenieModelConfig; timestamp: number } | null = null
  ...
  async getModelCapabilities(): Promise<ModelCapabilities> {
    ...
    if (provider.modelConfig?.contextWindow) {
      ...
      switch (provider.type) {
        case 'BrowserGenie':
          // BrowserGenie/BrowserGenie uses gemini 2.5 flash by default
          maxTokens = 1_000_000
          break
  ...
  getCurrentProvider(): BrowserGenieProvider | null {
  ...
  private _getDefaultModelForProvider(type: string, intelligence: string  = 'high'): string {
    switch (type) {
      case 'BrowserGenie':
        ...
        return DEFAULT_BrowserGenie_MODEL
      case 'openai':
      case 'openai_compatible':
      case 'openrouter':
      case 'custom':
  ...
  private async _createLLMFromProvider(
    ...
    switch (provider.type) {
      case 'BrowserGenie':
        // Only BrowserGenie uses intelligence parameter
        const intelligence = options?.intelligence ?? DEFAULT_INTELLIGENCE
        return await this._createBrowserGenieLLM(temperature, maxTokens, streaming, intelligence)
      ...
      default:
        Logging.log('LangChainProvider',
          `Unknown provider type: ${provider.type}, falling back to BrowserGenie`,
          'warning')
        const defaultIntelligence = options?.intelligence ?? DEFAULT_INTELLIGENCE
        return await this._createBrowserGenieLLM(temperature, maxTokens, streaming, defaultIntelligence)
    }
  }
  
  // Fetch model configuration from BrowserGenie API
  private async _fetchModelConfig(): Promise<BrowserGenieModelConfig> {
    if (this.modelConfigCache) {
    ...
    try {
      const response = await fetch(DEFAULT_BrowserGenie_MODEL_FAMILY_URL)
      if (response.ok) {
        const data = await response.json()

        // Ensure all fields are present with fallbacks
        const config: BrowserGenieModelConfig = {
          default: data.default || data.model_family || DEFAULT_BrowserGenie_MODEL,
          fast: data.fast || data.model_family || DEFAULT_BrowserGenie_MODEL,
          smart: data.smart || data.model_family || DEFAULT_BrowserGenie_MODEL
        }

        // Cache the result
        this.modelConfigCache = { config, timestamp: Date.now() }
        Logging.log('LangChainProvider',
          `BrowserGenie model config fetched - default: ${config.default}, fast: ${config.fast}, smart: ${config.smart}`,
          'info')
        return config
      }
    } catch (error) {
    ...
    const fallbackConfig: BrowserGenieModelConfig = {
      default: DEFAULT_BrowserGenie_MODEL,
      fast: DEFAULT_BrowserGenie_MODEL,
      smart: DEFAULT_BrowserGenie_MODEL
    }
    return fallbackConfig
  }

  // BrowserGenie built-in provider (uses proxy, no API key needed)
  private async _createBrowserGenieLLM(
    temperature: number,
    maxTokens?: number,
    ...
    if (intelligence === 'low') {
      ...
      proxyUrl = BrowserGenie_FAST_LLM_PROXY_URL
    } else {
      // Default to 'high' intelligence (smart provider)
      selectedProvider = modelConfig.smart
      proxyUrl = BrowserGenie_SMART_LLM_PROXY_URL
    }
