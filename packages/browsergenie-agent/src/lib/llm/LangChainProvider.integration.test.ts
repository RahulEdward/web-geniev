...
describe('LangChainProvider Integration Test', () => {
  ...
  it('should successfully create BrowserGenie LLM instance', async () => {
    // Note: This test validates the LLM instance creation
    // The internal configuration (proxy URL, API key) is handled by LangChain
    
    // Create a direct config for BrowserGenie provider
    const config = {
      provider: 'BrowserGenie' as const,
      model: 'gpt-4o-mini',
      temperature: 0,
      streaming: false,
      apiKey: process.env.LITELLM_API_KEY || 'nokey',
      baseURL: 'http://llm.BrowserGenie.ai'
    }
    ...
    console.log('✓ BrowserGenie LLM instance created successfully')
    console.log(`  - Provider: BrowserGenie (using LiteLLM proxy)`)
  ...
  it.skipIf(!process.env.LITELLM_API_KEY || process.env.LITELLM_API_KEY === 'nokey')(
    'should successfully invoke BrowserGenie LLM via LiteLLM proxy (requires LITELLM_API_KEY)',
    async () => {
      ...
      const response = await llm.invoke('Say "Hello from BrowserGenie!" and nothing else.')
