...
// Input schema for BrowserGenie info tool
const BrowserGenieInfoToolInputSchema = z.object({
  topic: z.enum([
    'overview',
    'installation',
    'configuration',
    ...
    'examples',
    'features'
  ]).describe('Information topic to retrieve about BrowserGenie'),
...
export type BrowserGenieInfoToolInput = z.infer<typeof BrowserGenieInfoToolInputSchema>

/**
 * BrowserGenieInfoTool - Provides comprehensive information about BrowserGenie features and usage
 * Contains various README-style documentation for different aspects of BrowserGenie
 */
export class BrowserGenieInfoToolImpl {
  ...
  async execute(input: BrowserGenieInfoToolInput): Promise<ToolOutput> {
    try {
      ...
      return toolError(`Failed to retrieve BrowserGenie info: ${error instanceof Error ? error.message : String(error)}`)
    }
  }

  private _getTopicInfo(topic: string, section?: string): string | null {
    const infoDatabase = {
      overview: `# BrowserGenie Overview

BrowserGenie is an AI-powered browser automation platform that enables intelligent web interaction through natural language commands.
...
- AI-assisted web navigation`,
      ...
      installation: `# BrowserGenie Installation Guide
...
git clone https://github.com/browsergenie-ai/BrowserGenie-agent
cd BrowserGenie-agent
...
4. The BrowserGenie extension should now appear in your extensions

### 3. Configuration
1. Click the BrowserGenie extension icon
...
\`\`\``,
      ...
      configuration: `# BrowserGenie Configuration
...
\`\`\``,
      ...
      automation: `# BrowserGenie Automation Capabilities
...
- User cancellation support`,
      ...
      api: `# BrowserGenie API Reference
...
\`\`\`javascript
// Execute a task
await BrowserGenie.execute("Navigate to Google and search for cats")

// Get browser state
const state = await BrowserGenie.getBrowserState()

// Take screenshot
const screenshot = await BrowserGenie.takeScreenshot()
\`\`\``,

      troubleshooting: `# BrowserGenie Troubleshooting
...
\`\`\`javascript
localStorage.setItem('BrowserGenie.debug', 'true')
\`\`\`
...
- Report issues with detailed reproduction steps`,
      ...
      examples: `# BrowserGenie Usage Examples
...
- Message: Hello from BrowserGenie
...
\`\`\``,
      ...
      features: `# BrowserGenie Features
...
/**
 * Factory function to create BrowserGenieInfoTool for LangChain integration
 */
export function BrowserGenieInfoTool(executionContext: ExecutionContext): DynamicStructuredTool {
  const tool = new BrowserGenieInfoToolImpl(executionContext)

  return new DynamicStructuredTool({
    name: "BrowserGenie_info_tool",
    description: `Get comprehensive information about BrowserGenie features, configuration, and usage.
    ...
    schema: BrowserGenieInfoToolInputSchema,
