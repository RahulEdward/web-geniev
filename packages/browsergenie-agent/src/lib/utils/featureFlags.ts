...
export class FeatureFlags {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      const adapter = getBrowserGenieAdapter()
      this.browserVersion = await adapter.getVersion()
      this.initialized = true
      Logging.log('FeatureFlags', `Initialized with BrowserGenie version: ${this.browserVersion}`)
    } catch (error) {
      Logging.log('FeatureFlags', `Failed to get BrowserGenie version, using defaults: ${error}`)
      this.initialized = true
    }
  }
