...
export class Logging {
  ...
  private static BrowserGenieAdapter = getBrowserGenieAdapter()
  ...
  /**
   * Log a metric event using the BrowserGenie metrics API with PostHog fallback
  ...
  public static async logMetric(eventName: string, properties?: Record<string, any>, sampling: number = 1.0): Promise<void> {
    ...
    try {
      await this.BrowserGenieAdapter.logMetric(prefixedEventName, enhancedProperties)
    } catch (error) {
      // BrowserGenie failed, use PostHog fallback
      if (this.posthogApiKey && this.posthogInitialized) {
        try {
