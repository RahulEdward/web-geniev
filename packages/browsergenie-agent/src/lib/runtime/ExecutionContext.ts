...
export const WSAgentConfigSchema = z.object({
  url: z.string().url(),  // Fallback URL if BrowserGenie preference not available
...
export class ExecutionContext {
  ...
  /**
   * Get WebSocket agent server URL from BrowserGenie preferences
   * Returns WebSocket URL using BrowserGenie.server.agent_port preference value
  ...
  public async getAgentServerUrl(): Promise<string> {
    ...
    try {
      const pref = await BrowserGenie.getPref('BrowserGenie.server.agent_port');

      if (pref && typeof pref.value === 'number') {
        const wsUrl = `ws://localhost:${pref.value}`;
        Logging.log(
          "ExecutionContext",
          `Using agent port from BrowserGenie preferences: ${pref.value}`,
          "info"
        );
        return wsUrl;
      }
      ...
      Logging.log(
        "ExecutionContext",
        `Failed to get agent port from BrowserGenie preferences: ${error}, using default URL: ${WS_AGENT_CONFIG.url}`,
        "error"
      );
      return WS_AGENT_CONFIG.url;
    }
