...
const BrowserGenie_USER_ID_KEY = 'BrowserGenie_user_id'
const PLATFORM_NAME = 'BrowserGenie'
...
export class KlavisAPIManager {
   ...
   * Format: BrowserGenie_<timestamp>_<random>
   */
  async getUserId(): Promise<string> {
    ...
    try {
      const storage = await chrome.storage.local.get([BrowserGenie_USER_ID_KEY])
      if (storage[BrowserGenie_USER_ID_KEY]) {
        this.userId = storage[BrowserGenie_USER_ID_KEY] as string
        return this.userId
      }
    } catch (error) {
    ...
    this.userId = `BrowserGenie_${timestamp}_${random}`
    
    // Save to Chrome storage
    try {
      await chrome.storage.local.set({ [BrowserGenie_USER_ID_KEY]: this.userId })
    } catch (error) {
