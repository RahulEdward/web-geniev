...
describe('KlavisAPIClient', () => {
  ...
  it('tests that client throws error when API key is empty', async () => {
    ...
    await expect(emptyClient.getUserInstances('user123', 'BrowserGenie'))
      .rejects
  ...
  it('tests that getUserInstances makes correct API call', async () => {
    ...
    const result = await client.getUserInstances('user123', 'BrowserGenie')
    
    expect(global.fetch).toHaveBeenCalledWith(
      'https://api.klavis.ai/user/instances?user_id=user123&platform_name=BrowserGenie',
  ...
  it('tests that API errors are handled properly', async () => {
    ...
    await expect(client.getUserInstances('user123', 'BrowserGenie'))
      .rejects
