...
import { type ScreenshotSizeKey } from "@/lib/browser/BrowserGenieAdapter";
import {
  AIMessage,
  AIMessageChunk,
  HumanMessage,
...
export class BrowserAgent {
  ...
  private async _registerTools(): Promise<void> {
    ...
    this.toolManager.register(BrowserGenieInfoTool(this.executionContext)); // BrowserGenie info tool
  ...
  private _getSpecialTaskMetadata(task: string): {task: string, metadata: ExecutionMetadata} | null {
    ...
    // BrowserGenie Launch Upvote Task
    if (taskLower === "read about our vision and upvote ❤️") {
      return {
        ...
        metadata: {
          ...
          predefinedPlan: {
            agentId: 'BrowserGenie-launch-upvoter',
            name: "BrowserGenie Launch Upvoter",
            goal: "Navigate to BrowserGenie launch page and upvote it",
            steps: [
              "Navigate to https://dub.sh/BrowserGenie-launch",
    ...
    // GitHub Star Task
    if (taskLower === "support BrowserGenie on github ⭐") {
      return {
        task: "Support BrowserGenie on GitHub",
        metadata: {
          executionMode: 'predefined' as const,
          predefinedPlan: {
            agentId: 'github-star-BrowserGenie',
            name: "GitHub Repository Star",
            goal: "Navigate to BrowserGenie GitHub repo and star it",
            steps: [
              "Navigate to https://git.new/BrowserGenie",
...
${actionsToExecute}
  ...
  private _formatPlannerOutputForExecutor(plan: PlannerOutput | PredefinedPlannerOutput): string {
    return `BrowserGenie Agent Output:
- Task: ${plan.userTask}
- Current State: ${plan.currentState}
- Execution History: ${plan.executionHistory}
- Challenges Identified: ${plan.challengesIdentified}
