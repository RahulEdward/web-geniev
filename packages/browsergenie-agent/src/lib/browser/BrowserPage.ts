...
export const InteractiveElementSchema = z.object({
  nodeId: z.number(), // Chrome BrowserGenie node ID (sequential index)
...
/**
 * BrowserPage - Simple browser page wrapper using Chrome BrowserGenie APIs
 *
 * This class provides:
 * 1. Direct element access via index-based APIs
 * 2. Element formatting for tools
 * 3. Simple action methods using BrowserGenieAdapter
 */
export class BrowserPage {
  private _tabId: number;
  private _url: string;
  private _title: string;
  private _BrowserGenie = getBrowserGenieAdapter();
  ...
  private async _getSnapshot(): Promise<InteractiveSnapshot | null> {
    return profileAsync("BrowserPage._getSnapshot", async () => {
      ...
      try {
        ...
        const snapshot = await this._BrowserGenie.getInteractiveSnapshot(
          this._tabId,
        );
  ...
  async getHierarchicalText(): Promise<string> {
    // Get accessibility tree from BrowserGenie adapter
    const tree = await this._BrowserGenie.getAccessibilityTree(this._tabId);
   ...
   */
  private _extractHierarchicalText(tree: chrome.BrowserGenie.AccessibilityTree): string {
  ...
  async clickElement(nodeId: number): Promise<void> {
    await profileAsync(`BrowserPage.clickElement[${nodeId}]`, async () => {
      ...
      await this._BrowserGenie.click(this._tabId, nodeId);
  ...
  async inputText(nodeId: number, text: string): Promise<void> {
    await profileAsync(`BrowserPage.inputText[${nodeId}]`, async () => {
      ...
      await this._BrowserGenie.clear(this._tabId, nodeId);
      await this._BrowserGenie.inputText(this._tabId, nodeId, text);
      await this.waitForStability();
      this._invalidateCache(); // Invalidate cache after stability
    });
  }
  ...
  async scrollToElement(nodeId: number): Promise<boolean> {
    return await this._BrowserGenie.scrollToNode(this._tabId, nodeId);
  }
  ...
  async sendKeys(keys: string): Promise<void> {
    // Define supported keys based on chrome.BrowserGenie.Key type
    const supportedKeys = [
      "Enter",
      "Delete",
      "Backspace",
    ...
    await this._BrowserGenie.sendKeys(this._tabId, keys as chrome.BrowserGenie.Key);
  ...
  async scrollDown(amount?: number): Promise<{ didScroll: boolean }> {
    ...
    for (let i = 0; i < scrollCount; i++) {
      await this._BrowserGenie.sendKeys(this._tabId, "PageDown");
  ...
  async scrollUp(amount?: number): Promise<{ didScroll: boolean }> {
    ...
    // Scroll the specified number of viewports with delay between each
    for (let i = 0; i < scrollCount; i++) {
      await this._BrowserGenie.sendKeys(this._tabId, "PageUp");
  ...
  async waitForStability(): Promise<void> {
    // CRITICAL: Add delay for Chrome BrowserGenie to update internal state after actions
    ...
    await profileAsync("BrowserPage.waitForStability", async () => {
      ...
      while (Date.now() - startTime < maxWaitTime) {
        try {
          const status = await this._BrowserGenie.getPageLoadStatus(this._tabId);
  ...
  async takeScreenshot(
    ...
    try {
      // Return the full data URL directly from BrowserGenie
      return await this._BrowserGenie.captureScreenshot(
        this._tabId,
        size,
        showHighlights,
      );
  ...
  async takeScreenshotWithDimensions(
    ...
    try {
      // Return the full data URL directly from BrowserGenie with exact dimensions
      return await this._BrowserGenie.captureScreenshot(
        this._tabId,
  ...
  async executeJavaScript(code: string): Promise<any> {
    try {
      return await this._BrowserGenie.executeJavaScript(this._tabId, code);
    } catch (error) {
      Logging.log(
        "BrowserPage",
  ...
  async clickAtCoordinates(x: number, y: number): Promise<void> {
    await profileAsync(
      ...
      async () => {
        ...
        await this._BrowserGenie.clickCoordinates(this._tabId, x, y);
        await this.waitForStability();
        this._invalidateCache(); // Invalidate cache after stability
      },
    );
  ...
  async typeAtCoordinates(x: number, y: number, text: string): Promise<void> {
    await profileAsync(`BrowserPage.typeAtCoordinates[${x},${y}]`, async () => {
      ...
      // await this._BrowserGenie.clickAtCoordinates(this._tabId, x, y);
      // Small delay to ensure focus is established
      // await new Promise(resolve => setTimeout(resolve, 100));
      // Then type the text
      await this._BrowserGenie.typeAtCoordinates(this._tabId, x, y, text);
      await this.waitForStability();
      this._invalidateCache(); // Invalidate cache after stability
    });
  }
  ...
  async showPointer(x: number, y: number, text?: string): Promise<void> {
    const pointerId = `BrowserGenie-pointer-${Date.now()}`;

    await this.executeJavaScript(`
      (function() {
        // Remove any existing pointer
        const existing = document.querySelector('.BrowserGenie-pointer');
        if (existing) existing.remove();
        
        // Create pointer container
        const pointer = document.createElement('div');
        pointer.className = 'BrowserGenie-pointer';
  ...
  async getTextSnapshot(options?: SnapshotOptions): Promise<Snapshot> {
    return await this._BrowserGenie.getTextSnapshot(this._tabId, options);
  }
  ...
  async getLinksSnapshot(options?: SnapshotOptions): Promise<Snapshot> {
    return await this._BrowserGenie.getLinksSnapshot(this._tabId, options);
  }
