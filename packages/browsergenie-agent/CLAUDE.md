...
- OUR PRODUCT NAME IS BrowserGenie -- so use that name correctly when naming things.
...
1. **BrowserGenie Core** (`src/lib/core/BrowserGenie.ts`) - Main orchestration layer that manages execution context and delegates to BrowserAgent
...
- **Provider Strategies**: AnthropicStrategy, OpenAIStrategy, OllamaStrategy, BrowserGenie Strategy
...
```
User Query → BrowserGenie.run() → BrowserAgent.execute()
                                        ↓
                              ClassificationTool
                                   ↙        ↘
...
1. **BrowserGenie.run()** - Main entry point that initializes execution context and calls BrowserAgent
...
- `src/lib/core/BrowserGenie.ts` - Main orchestration class
