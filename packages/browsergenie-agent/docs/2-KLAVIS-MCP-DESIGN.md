...
We are building an MCP (Model Context Protocol) integration for the BrowserGenie browser agent using Klavis as the third-party service provider. This integration will allow users to:
    ...
    try {
      ...
      switch (input.action) {
        case 'getUserInstances':
          ...
          const instances = await client.getUserInstances(userId, 'BrowserGenie')
...
export class KlavisAPIManager {
  ...
  async getUserId(): Promise<string> {
    ...
    const storage = await chrome.storage.local.get(['BrowserGenie_user_id'])
    if (storage.BrowserGenie_user_id) {
      this.userId = storage.BrowserGenie_user_id as string
      return this.userId
    }

    this.userId = `BrowserGenie_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
    await chrome.storage.local.set({ BrowserGenie_user_id: this.userId })
    return this.userId
  }
