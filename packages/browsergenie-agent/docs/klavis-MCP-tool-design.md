...
- **Seamless Integration**: MCP tools should work like native BrowserGenie tools
...
async function getUserId(): Promise<string> {
  ...
  if (storage.BrowserGenie_user_id) {
    return storage.BrowserGenie_user_id;
  }
  
  const userId = `BrowserGenie_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  await chrome.storage.local.set({ BrowserGenie_user_id: userId });
  return userId;
}
```
