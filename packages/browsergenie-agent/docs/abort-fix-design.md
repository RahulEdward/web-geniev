...
#### 3.1.5 BrowserGenie with State-Based Execution
```typescript
class BrowserGenie {
...
// Background handler - just coordinates with BrowserGenie
async function handleCancelTaskPort() {
  // BrowserGenie.cancel() will handle state transitions internally
  const result = await BrowserGenie.cancel()
...
// BrowserGenie orchestrates high-level flow
BrowserGenie {
  run() {
