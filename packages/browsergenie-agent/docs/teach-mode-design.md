# BrowserGenie Teach Mode - Design Document
...
| Feature | Chrome Recorder | BrowserGenie Teach Mode |
|---------|----------------|---------------------|
...
interface CapturedEvent {
  ...
  action: {
    ...
    target?: {
      nodeId: number;  // BrowserGenie nodeId
...
interface StateSnapshot {
  ...
  // BrowserGenie interactive snapshot
  interactiveSnapshot: BrowserGenieSnapshot;
...
## Missing BrowserGenie APIs

The following APIs need to be added to chrome.BrowserGenie to support teach mode:
...
chrome.BrowserGenie.startEventRecording(tabId: number, options: {
...
chrome.BrowserGenie.stopEventRecording(tabId: number): RecordedEvent[];

// Real-time event stream
chrome.BrowserGenie.onEventCaptured: Event<{
  tabId: number;
...
chrome.BrowserGenie.getElementContext(tabId: number, nodeId: number): {
  // Element hierarchy
  parentChain: Array<{
    tagName: string;
    text?: string;
...
chrome.BrowserGenie.startAudioRecording(options?: {
  format?: 'webm' | 'wav';
  quality?: 'low' | 'medium' | 'high';
}): void;

chrome.BrowserGenie.stopAudioRecording(): Promise<Blob>;

// Optional: Built-in transcription
chrome.BrowserGenie.transcribeAudio(
  audio: Blob,
  options?: {
    language?: string;
    model?: 'local' | 'cloud';
...
chrome.BrowserGenie.getAllTabsContext(): Promise<Array<{
  id: number;
  url: string;
  title: string;
  active: boolean;
...
chrome.BrowserGenie.onTabSwitched: Event<{
  fromTabId: number;
  toTabId: number;
  timestamp: number;
}>;
...
chrome.BrowserGenie.batchExecute(tabId: number, actions: Array<{
...
chrome.BrowserGenie.detectChanges(
...
chrome.BrowserGenie.findElementByDescription(
  tabId: number,
  description: string,
  options?: {
...
chrome.BrowserGenie.storage.workflows.save(
  workflow: SemanticWorkflow
): Promise<string>;  // workflow ID

chrome.BrowserGenie.storage.workflows.get(
  id: string
): Promise<SemanticWorkflow>;

chrome.BrowserGenie.storage.workflows.list(): Promise<Array<{
  id: string;
  name: string;
  createdAt: number;
}>>;
...
BrowserGenie Teach Mode represents a paradigm shift from brittle DOM-based recording to intelligent, semantic workflow automation. By combining voice narration, visual understanding, and adaptive execution, it enables users to create robust automations through natural demonstration rather than programming.

The phased implementation approach ensures we can deliver value quickly while building toward the complete vision. The architecture leverages existing BrowserGenie infrastructure while adding the key innovations needed for true learn-by-demonstration automation.
