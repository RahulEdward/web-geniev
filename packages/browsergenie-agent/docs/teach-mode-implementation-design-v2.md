...
This document outlines the implementation design for Teach Mode in the BrowserGenie browser agent. The design follows a KISS (Keep It Simple, Stupid) approach, leveraging existing patterns from the codebase (like GlowAnimationService) and Chrome's recorder implementation.
