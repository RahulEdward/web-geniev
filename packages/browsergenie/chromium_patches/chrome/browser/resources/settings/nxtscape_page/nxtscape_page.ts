diff --git a/chrome/browser/resources/settings/BrowserGenie_page/BrowserGenie_page.ts b/chrome/browser/resources/settings/BrowserGenie_page/BrowserGenie_page.ts
new file mode 100644
index 0000000000000..80e988de4d595
--- /dev/null
+++ b/chrome/browser/resources/settings/BrowserGenie_page/BrowserGenie_page.ts
@@ -0,0 +1,1099 @@
...
+
+/**
+ * @fileoverview 'settings-BrowserGenie-page' contains AI provider-specific settings.
+ */
+
...
+
+import {getTemplate} from './BrowserGenie_page.html.js';
+import {MODELS_DATA, getModelsForProvider, getModelContextLength} from './models_data.js';
+
+const SettingsBrowserGeniePageElementBase = PrefsMixin(PolymerElement);
+
+export enum ProviderType {
+  BrowserGenie = 'BrowserGenie',
+  OPENAI = 'openai',
...
+}
+
+export class SettingsBrowserGeniePageElement extends SettingsBrowserGeniePageElementBase {
+  static get is() {
+    return 'settings-BrowserGenie-page';
+  }
+
+  static get template() {
+    return getTemplate();
...
+    if (this.prefs && this.prefs.BrowserGenie) {
+      this.loadProviders_();
+    }
+  }
+
+  private loadProviders_() {
+    // Load from preferences or initialize with BrowserGenie
+    if (!this.prefs || !this.prefs.BrowserGenie || !this.prefs.BrowserGenie.providers) {
+      // Prefs not ready yet, initialize defaults locally only
+      this.initializeDefaultProvidersLocally_();
+      return;
+    }
+    
+    const stored = this.getPref('BrowserGenie.providers');
...
+        
+        // Ensure BrowserGenie is always present
+        if (!this.providers_.some(p => p.id === 'BrowserGenie')) {
...
+    
+    const BrowserGenie: ProviderConfig = {
+      id: 'BrowserGenie',
+      name: 'BrowserGenie',
+      type: ProviderType.BrowserGenie,
+      isDefault: true,
+      isBuiltIn: true,
+      createdAt: now,
+      updatedAt: now,
+    };
+    
+    this.providers_ = [BrowserGenie];
+    this.defaultProviderId_ = 'BrowserGenie';
...
+    if (this.prefs && this.prefs.BrowserGenie && this.prefs.BrowserGenie.providers !== undefined) {
+      this.saveProviders_();
+    }
+  }
+
...
+    if (!this.prefs || !this.prefs.BrowserGenie || this.prefs.BrowserGenie.providers === undefined) {
+      console.warn('BrowserGenie: Cannot save providers - prefs not ready');
+      return;
+    }
+    
...
+    this.setPrefValue('BrowserGenie.providers', JSON.stringify(data));
+    console.log('BrowserGenie: Saving providers:', data);
+  }
+
...
+    console.log('BrowserGenie: Add Provider clicked');
+    
...
+    }
+    
+    console.log('BrowserGenie: Form visibility:', this.showProviderForm_);
+  }
+
...
+    const provider = this.providers_.find(p => p.id === providerId);
+    
+    console.log('BrowserGenie: Delete provider clicked:', providerId, provider);
+    
...
+      
+      // If deleted provider was default, set BrowserGenie as default
+      if (provider.id === this.defaultProviderId_) {
+        this.set('defaultProviderId_', 'BrowserGenie');
...
+    }
+    
+    console.log('BrowserGenie: Provider type changed to:', providerType);
+    
+    const defaults = PROVIDER_DEFAULTS[providerType];
+    if (defaults) {
+      console.log('BrowserGenie: Applying defaults:', defaults);
...
+            console.log('BrowserGenie: Auto-set context length to:', contextLength);
+          }
+          
+          this.set('showModelDropdown_', false);
+          this.set('selectedSuggestionIndex_', -1);
...
+    console.log('BrowserGenie: Model suggestion clicked:', model);
+    
+    if (model) {
+      console.log('BrowserGenie: Setting dialogModelId_ to:', model);
...
+      if (input) {
+        input.value = model;
+        console.log('BrowserGenie: Input value set to:', input.value);
+      }
+    }
+  }
+  
...
+    console.log('BrowserGenie: Custom model selected:', this.dialogModelId_);
+    
...
+      
+      chrome.send('logBrowserGenieMetric', ['settings.provider.added', {
...
+    console.log('BrowserGenie: Validating form:', {
+      name: this.dialogProviderName_,
+      type: this.dialogProviderType_,
+      apiKey: this.dialogApiKey_,
+      modelId: this.dialogModelId_
...
+
+    chrome.send('logBrowserGenieMetric', ['settings.default_provider.changed', {
...
+      classes += ' selected';
+    }
+    if (provider.type === ProviderType.BrowserGenie) {
+      classes += ' BrowserGenie';
+    }
+    return classes;
+  }
+  
...
+    }, 600);
+    
+    console.log('BrowserGenie: Using template:', template.name);
+  }
+
...
+    'settings-BrowserGenie-page': SettingsBrowserGeniePageElement;
+  }
+}
+
+customElements.define(
+    SettingsBrowserGeniePageElement.is, SettingsBrowserGeniePageElement);
