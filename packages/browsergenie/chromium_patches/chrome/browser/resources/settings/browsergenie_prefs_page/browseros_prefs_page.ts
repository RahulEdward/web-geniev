diff --git a/chrome/browser/resources/settings/BrowserGenie_prefs_page/BrowserGenie_prefs_page.ts b/chrome/browser/resources/settings/BrowserGenie_prefs_page/BrowserGenie_prefs_page.ts
new file mode 100644
index 0000000000000..2616c77cc001b
--- /dev/null
+++ b/chrome/browser/resources/settings/BrowserGenie_prefs_page/BrowserGenie_prefs_page.ts
@@ -0,0 +1,250 @@
...
+
+/**
+ * @fileoverview 'settings-BrowserGenie-prefs-page' contains BrowserGenie-specific settings.
+ */
+
...
+
+import {getTemplate} from './BrowserGenie_prefs_page.html.js';
+
...
+}
+
+export interface SettingsBrowserGeniePrefsPageElement {
+  $: {
+    addProviderDialog: HTMLElement;
+  };
+}
+
+const SettingsBrowserGeniePrefsPageElementBase = PrefsMixin(PolymerElement);
+
+export class SettingsBrowserGeniePrefsPageElement extends SettingsBrowserGeniePrefsPageElementBase {
+  static get is() {
+    return 'settings-BrowserGenie-prefs-page';
+  }
+
+  static get template() {
+    return getTemplate();
...
+    try {
+      const pref = this.getPref('BrowserGenie.custom_providers');
+      if (pref && pref.value) {
...
+    this.setPrefValue('BrowserGenie.custom_providers', customProvidersJson);
+  }
+  
+  /**
...
+    'settings-BrowserGenie-prefs-page': SettingsBrowserGeniePrefsPageElement;
+  }
+}
+
+customElements.define(
+    SettingsBrowserGeniePrefsPageElement.is, SettingsBrowserGeniePrefsPageElement);
